...
2. Subscribe to `public:trade_orders` through Supabase Realtime with the access token.
3. Treat each Realtime event as a notification, then fetch `GET /v1/ea/orders`.
...
After startup, token refresh, or any Realtime reconnect, call `GET /v1/ea/orders`.
This recovery request is required because Realtime delivery is at-least-once and
events can be missed while the EA is offline.
