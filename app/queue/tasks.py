...
def _run(prompt: str, messages: list[dict], connectors: list[str] | None = None) -> dict:
    return asyncio.run(run_agent_loop(prompt, messages, connectors or []))


@celery_app.task(name="app.queue.tasks.run_paid_job", bind=True, max_retries=2)
def run_paid_job(self, prompt: str, messages: list[dict], connectors: list[str] | None = None):
    try:
        return _run(prompt, messages, connectors)
    except Exception as exc:
        raise self.retry(exc=exc, countdown=2)


@celery_app.task(name="app.queue.tasks.run_free_job", bind=True, max_retries=1)
def run_free_job(self, prompt: str, messages: list[dict], connectors: list[str] | None = None):
    try:
        return _run(prompt, messages, connectors)
    except Exception as exc:
